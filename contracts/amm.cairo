...
# ######## Getters
@view
func get_token_a{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    pool_id : felt
) -> (token_a : felt):
...
@view
func get_token_b{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    pool_id : felt
) -> (token_b : felt):
    let (pool_core_instance) = pools_storage.read(pool_id)
...
@view
func get_balance_token{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    pool_id : felt, token : felt
) -> (balance : felt):
...
@view
func get_number_of_pools{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}() -> (
    number_of_pools : felt
):
...
func swap{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    pool_id : felt, token_amount : felt, token_address : felt
) -> (amount : felt):
    let (sender_address) = get_caller_address()
    let (user_balance_256) = IERC20.balanceOf(
        contract_address=token_address, account=sender_address
    )
...
func provide_liquidity{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    pool_id : felt, token_a_amount : felt, token_b_amount : felt
):
    let (sender_address) = get_caller_address()
    let (pool_core_instance) = pools_storage.read(pool_id)
    let (contract_address) = get_contract_address()
    ...
    IERC20.transferFrom(
        ...
        sender=sender_address,
        recipient=contract_address,
        amount=Uint256(token_a_amount, 0),
    )
    IERC20.transferFrom(
        contract_address=token_b,
        sender=sender_address,
        recipient=contract_address,
        amount=Uint256(token_b_amount, 0),
    )
